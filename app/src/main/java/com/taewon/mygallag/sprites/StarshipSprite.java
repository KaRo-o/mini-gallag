...
public class StarshipSprite extends Sprite{
    ...
    public void init() { // 스프라이트 정보 초기화
        ...
        Integer [] shots = {R.drawable.shot_001,R.drawable.shot_002,
                R.drawable.shot_003,R.drawable.shot_004,R.drawable.shot_005,
                R.drawable.shot_006,R.drawable.shot_007}; // 탄환 스프라이트 리소스
    ...
    public int getBulletsCount() {return bullets;} // 현재 남은 총알 개수를 알기위해 존재(메인액티비티로 보내줌)

    public void moveRight (double force) { setDx((float)(1*force*speed));} // force => 매개변수
    public void moveLeft (double force) { setDx((float)(-1*force*speed));} // 현재 속도(speed)와 매개변수(force) 값으로 속도를 구한 뒤 해당 방향으로 이동하게함
    ...
    public void powerUp() {
        ...
        powerLevel++; // 그렇지 않을경우 파워레벨을 증가 시킨 뒤
        MainActivity.fireBtn.setImageResource(bulletSprites.get(powerLevel)); // 파워업한 현재 총알 이미지로 발사 버튼의 이미지도 변경함
        MainActivity.fireBtn.setBackgroundResource(R.drawable.round_button_shape); // <= 왜 있는지?
    }
    ...
    public void reloadBullets() {
        ...
        new Handler().postDelayed(new Runnable() { // 일정시간 지난후에 아래 작업을 수행하게 딜레이를 줌
            @Override
            public void run() {
                bullets = 30;
                ...
                MainActivity.bulletCount.invalidate(); // invalidate => 호출시 변경된 ui를 적용시킴
                isReloading = false;
            }
        },2000); // 1밀리세컨드 = 0.001초 
    }
    ...
    public void specialShot(){
        ...
        SpecialshotSprite shot = new SpecialshotSprite(context, game, R.drawable.laser, // 스페셜 샷 스프라이트 추가
                getRect().right - getRect().left, 0 ); // 스프라이트 이미지 기준으로 좌표 잡힘
    ...
    public void hurt(){
        life--; // 체력 1 감소
        if(life<=0){
            ((ImageView) MainActivity.lifeFrame.getChildAt(life)).setImageResource( //체력이 0 이하일경우 게임 종료
                    (R.drawable.ic_baseline_favorite_border_24)
            );
            game.endGame();
            return;
        }
        ((ImageView) MainActivity.lifeFrame.getChildAt(life)).setImageResource( // 체력이 남아있을 경우 이미지 변경
                (R.drawable.ic_baseline_favorite_border_24)
        );
    }
    ...
    public void heal(){
        ...
        ((ImageView) MainActivity.lifeFrame.getChildAt(life)).setImageResource( // 회복할 체력 있으면 해당 체력의 이미지뷰를 변경
